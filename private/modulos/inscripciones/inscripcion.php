...
class inscripciones{
    ...
    private function administrar_inscripciones(){
        ...
        if($accion==='nuevo'){
            return $this->db->consultaSQL('INSERT INTO inscripciones (idInscripcion, idAlumno, idMateria, ciclo, fecha) VALUES (?, ?, ?, ?, ?)',
            $this->datos['idInscripcion'], $this->datos['idAlumno'], $this->datos['idMateria'], $this->datos['ciclo'], $this->datos['fecha']);
        }else if($accion==='modificar'){
            return $this->db->consultaSQL('UPDATE inscripciones SET idAlumno = ?, idMateria = ?, ciclo = ?, fecha = ? WHERE idInscripcion = ?',
            $this->datos['idAlumno'], $this->datos['idMateria'], $this->datos['ciclo'], $this->datos['fecha'], $this->datos['idInscripcion']);
            ...
            $this->db->consultaSQL('
                SELECT idInscripcion, idAlumno, idMateria, ciclo, fecha 
                FROM inscripciones
            ');
