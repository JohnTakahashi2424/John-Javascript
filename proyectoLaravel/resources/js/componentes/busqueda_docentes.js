                ...
                <div class="card shadow-sm border-0 rounded-4 mb-4 bg-body">
                    ...
                    <div class="card-body p-3">
                        <div class="input-group input-group-sm mb-3 shadow-sm rounded-pill overflow-hidden">
                            ...
                            <input autocomplete="off" type="search" @keyup="obtenerDocentes()" v-model="buscar" placeholder="Buscar por código, nombre o correo..." class="form-control bg-body-tertiary border-0 px-3 text-body shadow-none py-1">
                        </div>
                            ...
                            <table class="table table-sm fs-6 table-hover align-middle mb-0" id="tblDocentes">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="py-2 border-bottom-0 text-secondary fw-semibold">ESCALAFÓN</th>
                                ...
                                <tbody class="border-top-0">
                                    <tr v-for="docente in docentes" :key="docente.idDocente" @click="modificarDocente(docente)" class="cursor-pointer transition-all">
                                        ...
                                        <td class="text-capitalize"><span class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 fs-6 rounded-pill px-3">{{ docente.escalafon }}</span></td>
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <button @click.stop="modificarDocente(docente)" class="btn btn-outline-info btn-sm rounded-pill shadow-sm px-2 me-1">
                                                    <i class="bi bi-pencil-fill"></i>
                                                </button>
                                                <button @click.stop="eliminarDocente(docente, $event)" class="btn btn-outline-danger btn-sm rounded-pill shadow-sm px-2">
                                    ...
                                    <tr v-if="docentes.length == 0">
                                        <td colspan="7" class="text-center text-muted py-3">No se encontraron docentes...</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
