                ...
                <div class="card shadow-sm border-0 rounded-4 mb-4 bg-body">
                    ...
                    <div class="card-body p-3">
                        <div class="input-group input-group-sm mb-3 shadow-sm rounded-pill overflow-hidden">
                            ...
                            <input autocomplete="off" type="search" @keyup="obtenerMaterias()" v-model="buscar" placeholder="Buscar por código o nombre..." class="form-control bg-body-tertiary border-0 px-3 text-body shadow-none py-1">
                        </div>
                            ...
                            <table class="table table-sm fs-6 table-hover align-middle mb-0" id="tblMaterias">
                                ...
                                <tbody class="border-top-0">
                                    <tr v-for="materia in materias" :key="materia.idMateria" @click="modificarMateria(materia)" class="cursor-pointer transition-all">
                                        ...
                                        <td><span class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 fs-6 rounded-pill px-3">{{ materia.uv }}</span></td>
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <button @click.stop="modificarMateria(materia)" class="btn btn-outline-info btn-sm rounded-pill shadow-sm px-2 me-1">
                                                    <i class="bi bi-pencil-fill"></i>
                                                </button>
                                                <button @click.stop="eliminarMateria(materia, $event)" class="btn btn-outline-danger btn-sm rounded-pill shadow-sm px-2">
